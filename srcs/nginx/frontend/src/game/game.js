...
export class Game {
	...
	constructor() {
		...
		this.#score = new Score({
			...
			fontPath: "../public/font/Poppins ExtraBold_Regular.json",
	...
	refreshBallPos() {
		// handling collision detection with paddles and sides of the field
		this.#ball.move(); // refresh the ball position

		// switch the Z direction of the ball if it hits the top or bottom side
		if (this.#ball.getPosZ() + BALL_RADIUS >= FIELD_DIMENSION_Z / 2 || this.#ball.getPosZ() - BALL_RADIUS <= -(FIELD_DIMENSION_Z / 2)) {
		...
		if (this.#ball.getPosX() - BALL_RADIUS <= this.#paddleLeft.getPosX() + PADDLE_DIMENSION_X / 2 && this.#ball.getPosZ() >= this.#paddleLeft.getPosZ() - PADDLE_DIMENSION_Z / 2 && this.#ball.getPosZ() <= this.#paddleLeft.getPosZ() + PADDLE_DIMENSION_Z / 2) {
		...
		if (this.#ball.getPosX() + BALL_RADIUS >= this.#paddleRight.getPosX() - PADDLE_DIMENSION_X / 2 && this.#ball.getPosZ() >= this.#paddleRight.getPosZ() - PADDLE_DIMENSION_Z / 2 && this.#ball.getPosZ() <= this.#paddleRight.getPosZ() + PADDLE_DIMENSION_Z / 2) {
